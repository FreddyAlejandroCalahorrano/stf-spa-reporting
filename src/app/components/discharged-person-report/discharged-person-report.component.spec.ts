import {ComponentFixture, TestBed} from '@angular/core/testing';

import {DischargedPersonReportComponent} from './discharged-person-report.component';
import {HttpModule} from "@pichincha/angular-sdk/http";
import {FormControl, FormGroup, FormsModule, ReactiveFormsModule} from "@angular/forms";
import {CustomCommonModule} from "../../common/components/common.module";
import {ProvidersService} from "@services/providers.service";
import {CelulaService} from "@services/celula.service";
import {TribuService} from "@services/tribu.service";
import {ProfileService} from "@services/profile.service";
import {CUSTOM_ELEMENTS_SCHEMA} from "@angular/core";
import {DischargedPersonReportService} from "@services/discharged-person-report.service";

describe('DischargedPersonReportComponent', () => {
  let component: DischargedPersonReportComponent;
  let fixture: ComponentFixture<DischargedPersonReportComponent>;

  let mockCelulaService = {
    getCelulaByArrayTribu: jest.fn(),
  }
  let mockDischargedPersonReportService = {
    getDischargedPersonSearchPaged: jest.fn(),
    getDischargedReportPersonExcel: jest.fn()
  }
  let mockTribuService = {
    getTribu: jest.fn().mockResolvedValue([]),
  }
  let mockProfileService = {
    getProfiles: jest.fn().mockResolvedValue([]),
  }

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [
        HttpModule.forRoot({api_url: ""}),
        FormsModule,
        ReactiveFormsModule,
        CustomCommonModule,
      ],
      declarations: [ DischargedPersonReportComponent ],
      providers: [
        ProvidersService,
        {provide: CelulaService, useValue: mockCelulaService},
        {provide: DischargedPersonReportService, useValue: mockDischargedPersonReportService},
        {provide: TribuService, useValue: mockTribuService},
        {provide: ProfileService, useValue: mockProfileService},
      ],
      schemas: [CUSTOM_ELEMENTS_SCHEMA]
    })
    .compileComponents();
  });

  beforeEach(() => {
    fixture = TestBed.createComponent(DischargedPersonReportComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });

  it('check initial values form group filters', () => {
    expect(component.celulaDP?.value).toEqual([]);
    expect(component.profileDP?.value).toEqual([]);
    expect(component.providerDP?.value).toEqual([]);
    expect(component.filter?.value).toEqual(null);
    expect(component.tribuDP.value).toEqual([]);
    expect(component.asignmentEndDateDP?.get('from').value).toEqual(null);

  });

  it('should called method "getCelulaByArrayTribu" when change value control tribu', () => {

    const spyCelula = mockCelulaService.getCelulaByArrayTribu
      .mockImplementation(() => Promise.resolve([]))

    component.tribuDP.setValue([1, 2, 3])
    expect(
      component.celulaDP.value
    ).toHaveLength(0);

    expect(
      spyCelula
    ).toBeCalled()
  })

  it('should called method "next" from [dataReport$] when call onClickSearch', () => {
    const spyNextSubject = jest.spyOn(component.dataReport$, 'next')
    component.search()
    expect(
      spyNextSubject
    ).toBeCalled()
    expect(
      component.currentPage
    ).toEqual(1)
  })

  it('should called method "next" from [dataReport$] when call setupPagination', () => {
    const spyNextSubject = jest.spyOn(component.dataReport$, 'next')
    component.setupPaginationDischargedPerson({sizePage: 5, currentPage: 1, start: 0, end: 0})
    expect(
      spyNextSubject
    ).toBeCalled()
  })

  it('should return value form group filters, valid to query', () => {
    component.filtersDischargedPerson.patchValue({
      tribu: [1, 2, 3],
      celula: null,
      profile: null,
      asignmentEndDateDP: {
        from: null,
        to: null,
      },
    }, {emitEvent: false})

    component.filtersDischargedPerson.patchValue({
      asignmentEndDateDP: {
        from: '2022-02-10',
        to: '2022-04-30',
      },
    })
    const renewValueForm = component.getFiltersValue()
    expect(
      renewValueForm.tentativeEndRange
    ).toEqual({
      from: '2022-02-10',
      to: '2022-04-30',
    })

  })

  it('should give null in fn[validateDates] if all dates are null', () => {
    let formGroup = new FormGroup({
      'from': new FormControl(null),
      'to': new FormControl(null),
    }, {
      validators: component.validateDatesDischargedPerson()
    });
    expect(formGroup.hasError('validateDates')).toBeFalsy()
  })

  it('should give required error message in fn[validateDates] if any date is null', () => {
    let formGroup = new FormGroup({
      'from': new FormControl('2022-05-18'),
      'to': new FormControl(null),
    }, {
      validators: component.validateDatesDischargedPerson()
    });
    expect(formGroup.hasError('validateDates')).toBeTruthy()
  })

  it('should give null in fn[validateDates] if none date is null', () => {
    let formGroup = new FormGroup({
      'from': new FormControl('2022-05-18'),
      'to': new FormControl('2022-05-20'),
    }, {
      validators: component.validateDatesDischargedPerson()
    });
    expect(formGroup.hasError('validateDates')).toBeFalsy()
  })

  it('should give range error message in fn[validateDates]', () => {
    let formGroup = new FormGroup({
      'from': new FormControl('2022-05-18'),
      'to': new FormControl('2022-05-17'),
    }, {
      validators: component.validateDatesDischargedPerson()
    });
    expect(formGroup.hasError('validateDates')).toBeTruthy()
  });

  /*it('should get the assigned date "asignmentStartDate"', () => {
    const data = {
      detail:[
        {
          dateFrom: new Date("Tue Jun 07 2022 00:00:00 GMT-0500") ,
          dateUntil: new Date("Tue Jun 07 2022 00:00:00 GMT-0500")
        }
      ]
    }
    component.calendarSelectAssignment(data)
    fixture.detectChanges()
    expect(component.asignmentStartDate.get('from').value).toEqual('2022-06-07')
  });*/

  it('should get the assigned date "tentativeAsignmentEndDate"', () => {
    const data = {
      detail:[
        {
          dateFrom: new Date("Tue Jun 07 2022 00:00:00 GMT-0500") ,
          dateUntil: new Date("Tue Jun 07 2022 00:00:00 GMT-0500")
        }
      ]
    }
    component.calendarSelectEndDate(data)
    fixture.detectChanges()
    expect(component.asignmentEndDateDP.get('from').value).toEqual('2022-06-07')
  });

  it('should call the method "getReportPersonExcel"', () => {
    const data = {
      "fileReportTo": {
        "fileName": "Staff Desarrollo.xlsx",
        "fileBase64": "",
        "mimeType": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
      },
    }
    const spy = mockDischargedPersonReportService.getDischargedReportPersonExcel
      .mockImplementation(() => Promise.resolve([data]))
    component.onClickSearchDischargedPerson()
    expect(spy).toBeCalled()

  });

  it('should change the state of the variable "showMessageBarDischargedPerson" to false', () => {
    const event = {
      "isTrusted": false
    }
    component.onClickMessageDischargedPerson(event)
    expect(component.showMessageBarDischargedPerson).toBeFalsy()
  });


});
