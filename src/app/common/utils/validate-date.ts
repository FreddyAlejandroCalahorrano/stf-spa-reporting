export function validateDate(dateValue: string, responseType = "boolean", dateFormat: any = null) {
...
function responseSetter(responseType: string | boolean): string[] | boolean[] {
  switch (responseType) {
    case "string":
...
function dateValidator(dateValue: string, responses: string[] | boolean[], dateFormat: string) {
  if (dateValue) {
    ...
    let dayPosition: any = getAllIndexes(
      formatSplit,
      formatSplit.filter((formatPart) => /[d]/i.test(formatPart))[0]
    );
    let monthPosition: any = getAllIndexes(
      formatSplit,
      formatSplit.filter((formatPart) => /[m]/i.test(formatPart))[0]
    );
    let yearPosition: any = getAllIndexes(
      formatSplit,
